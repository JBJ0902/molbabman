    ...
    function showIdleStageNotice({ title = "딴짓 경고", text = "", count = "", mode = "warn", duration = 1800 } = {}) {
      const notice = document.getElementById("idleStageNotice");
      if (!notice) return;
      ...
      notice.classList.remove("hidden", "info", "rocket");
    ...
    function updateIdleStageTimer(dt) {
      ...
      if (!startOverlay.classList.contains("hidden") || !endOverlay.classList.contains("hidden") || !optionsOverlay.classList.contains("hidden")) {
        game.idleEatTimer = 0;
        return;
      }

      if (typeof isOnlineArenaOpen === "function" && isOnlineArenaOpen()) {
        game.idleEatTimer = 0;
        return;
      }
    ...
    function stageRequirementFail() {
      ...
      if (game.objectiveFailCount >= 5) {
        ...
        showIdleStageNotice({
          title: "딴짓 최종 경고",
          text: "20초 무입력 경고가 5회 누적되어 로켓 방출 엔딩으로 이동합니다.",
          count: failText,
          mode: "rocket",
          duration: 1900
        });
        ...
        pulseWrap("danger");
        showResultBanner("로켓 방출!", "danger", 1250);
        spawnResultParticles("gameover");
        playSfx("rocket", 0.85);
        ...
        statusText.innerHTML = `딴짓 경고 <b>5회</b> 누적 · 로켓 방출`;
        updateHUD();
      ...
      showIdleStageNotice({
        title: "딴짓 경고",
        text: "20초 동안 밥/김치 입력이 없어 현재 스테이지를 다시 시작합니다.",
        count: failText,
        mode: "warn",
        duration: 1900
      });
      ...
      pulseWrap("danger");
      spawnResultParticles("gameover");
      playSfx("warning", 0.58);

      statusText.innerHTML = `20초 무입력 · 딴짓 경고 <b>${failText}</b><br>현재 스테이지를 다시 시작합니다.`;
      updateHUD();

      restartStageAfterSmallNotice(`스테이지 ${game.stage} 재도전 · 20초 이상 손을 놓지 마세요`);
    }

    ...
    function stageRequirementFail() {
      ...
      pulseWrap("danger");
      showResultBanner("목표 미달!", "danger", 1250);
      spawnResultParticles("gameover");
      playSfx("warning", 0.72);

      const scoreNow = Math.floor(game.score || 0).toLocaleString();
      statusText.innerHTML = `몰밥 실적 부족! 목표 <b>${req.minScore.toLocaleString()}점</b> / <b>${req.minEatCount}회</b>를 채우지 못했습니다.<br>현재 ${scoreNow}점 · 몰밥 ${game.eatCount || 0}회. 목숨 -1 후 같은 스테이지를 재도전합니다.`;
      ...
      game.stageFailTimer = setTimeout(() => {
        ...
        statusText.textContent = `스테이지 ${game.stage} 재도전. 제한 시간 안에 목표 점수와 몰밥 횟수를 채우세요.`;
        updateHUD();

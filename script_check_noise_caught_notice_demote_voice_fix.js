    ...
    function determineEnding(clear) {
      ...
      if (!clear) {
        unlockEnding("rocket");
        return { key: "rocket", title: game.stage >= 7 ? "장렬한 로켓 방출 엔딩" : "조기 방출 엔딩", desc: `로켓에 실려 크루에서 방출되었습니다.<br>최종 점수: <b>${finalScore.toLocaleString()}</b><br>최종 계급: <b>방출</b><br>최대 콤보: <b>${game.maxCombo}</b>`, voice: "로켓 방출 엔딩입니다. 다음에는 몰밥할 때 들키지 마세요." };
      }
      if (game.life === 5 && game.rankIndex === 0) {
        unlockEnding("perfect");

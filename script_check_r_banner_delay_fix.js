    ...
    function setSafePhase() {
      ...
      const cfg = getStageConfig(game.stage);
      // HYBRID: 수장님이 공태연을 볼 때는 긴 경고/변명 음성을 재생하지 않고, 화면 경고와 실제 적발 음성만 사용합니다.
      if (HYBRID_AUDIO_EVENT_ONLY && dialogueState.busy && !dialogueState.protectedSequence) cancelDialogue(false, true);
      game.phase = "watch";
    ...
    function resetCurrentStageByHotkey() {
      ...
      // R 키 재시작 전용 흐름:
      // 재시작 자막을 충분히 보여준 뒤 STAGE START 카드를 표시합니다.
      // 정상 스테이지 전환의 START 카드 타이밍은 건드리지 않습니다.
      showResultBanner(`STAGE ${stage} 재시작`, "good", 1600);
      ...
      clearTimeout(game._stageStartTimer);
      setTimeout(() => {
        if (!game.running || game.stage !== stage || game.phase === "launching" || game.phase === "finished") return;

        showStageStartCard(stage);

        game._stageStartTimer = setTimeout(() => {
          if (game.running && game.stage === stage && game.phase !== "launching" && game.phase !== "finished") {
            setSafePhase();
          }
        }, 1100);
      }, 1750);

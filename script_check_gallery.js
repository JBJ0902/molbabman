    ...
    function stageClear() {
      ...
      statusText.textContent = `스테이지 ${game.stage} 회의 시작. ${formatClock(game.stageTimeLimit)} 동안 버티면서 점수를 쌓으세요. 이전 스테이지까지 누적 점수: ${Math.floor(game.totalScore).toLocaleString()}점`;
      updateHUD();
    ...
    function update(dt) {
      if (!game.running) return;
      game.phaseTimer -= dt;
      if (!["stageClear", "launching"].includes(game.phase)) {
      ...
      updateLookTimerHUD();
      updateStageTimerHUD();
      if (game.stageTimeLeft <= 0 && game.phase !== "stageClear" && game.phase !== "launching") { stageClear(); return; }
      if (game.phase === "safe" && game.phaseTimer <= 0) setPrewarningPhase();
      else if (game.phase === "prewarning" && game.phaseTimer <= 0) setWatchPhase();
      else if (game.phase === "watch" && game.phaseTimer <= 0) setSafePhase();
      else if (game.phase === "caught" && game.phaseTimer <= 0 && game.life > 0) setSafePhase();

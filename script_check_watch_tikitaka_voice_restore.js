    ...
    const WATCH_DIALOGUE_USES_VOICE = true;
    ...
    const WATCH_DIALOGUE_COOLDOWN_MS = 4600;
    ...
    async function showWatchDialogueSequence() {
      ...
      try {
        ...
        await createBubble(bubblePeople[0], watchLine.text, WATCH_DIALOGUE_USES_VOICE, watchLine.source, {
          token,
          afterGapMs: 140,
          textOnlyVisibleMs: 1900,
          voiceOptions: { interrupt: true }
        });
    ...
    function update(dt) {
      ...
      else if (game.phase === "watch" && game.phaseTimer <= 0) {
        // 공태연 WATCH 티키타카 자막/음성이 재생 중이면 중간에 끊지 않고,
        // 수장님 시선을 공태연에게 유지한 뒤 대화가 끝나면 안전 구간으로 넘어갑니다.
        if (dialogueState.protectedSequence) {
          game.phaseTimer = 0.25;
          setGaze("player");
        } else {
          setSafePhase();
        }

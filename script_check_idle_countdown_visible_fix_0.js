    ...
    const IDLE_STAGE_PREWARN_SECONDS = 17; // 20초 중 3초 경과 후부터 카운트다운 표시
    ...
    function updateIdleStageTimer(dt) {
      ...
      // 20초 중 3초가 지나면 바로 카운트다운을 보여줍니다.
      // 이전처럼 마지막 10초에만 보이지 않게 하여 사용자가 확실히 인지할 수 있게 합니다.
      if (game.idleEatTimer >= 3 && game.idleEatTimer < IDLE_STAGE_WARNING_SECONDS) {
        showIdleStageNotice({
          title: "딴짓 경고 카운트다운",
    ...
    function updateStartOnlineChip() {
      ...
      if (onlineState.user && onlineState.profile) {
        ...
        chip.textContent = `온라인 순위전: Google 로그인 필요 · 접속자 ${onlineState.activeCount || 0}명`;
      }
    }
    ...
    function updateOnlineProfileUI() {
      ...
      if (!loggedIn) {
        setOnlineAuthStatus("Google 로그인 후 온라인 순위전, 티어, 엔딩 해금 저장을 사용할 수 있습니다.", "warn");
    ...
    async function handleGoogleLogin() {
      ...
      if (error) setOnlineAuthStatus(`Google 로그인 시작 실패: ${error.message}`, "err");
    }
    ...
    function startOnlineRankedGame() {
      if (!onlineState.user) {
        setOnlineAuthStatus("순위전은 Google 로그인 후 시작할 수 있습니다.", "err");
        return;
      }
    ...
    async function submitOnlineScore(ending) {
      ...
      if (!onlineState.user) {
        setOnlineSaveStatus("Google 로그인 상태가 아니어서 온라인 순위전 기록을 저장하지 못했습니다.", "err");
        await loadOnlineRanking();
        return;
      }

    ...
    const bubblePeople = [
      { id: "leader", name: "수장", x: 49.6, y: 18.8, px: 40.6, py: 20.2, role: "leader", cls: "speaker-leader", pitch: 0.82, rate: 1.0 },
    ...
    function determineEnding(clear) {
      if (!clear) {
        ...
        return {
          ...
          desc: `로켓에 실려 팀에서 방출되었습니다.<br>최종 점수: <b>${Math.floor(game.score).toLocaleString()}</b><br>최종 계급: <b>${RANKS[Math.min(game.rankIndex, RANKS.length - 1)]}</b><br>최대 콤보: <b>${game.maxCombo}</b>`,
      ...
      if (game.life === 5 && game.rankIndex === 0) {
        ...
        return {
          key: "perfect",
          title: "레전드 정직원 엔딩",
          desc: `한 번도 들키지 않고 고난도 장기전 10스테이지를 완벽히 돌파했습니다!<br>최종 점수: <b>${Math.floor(game.score).toLocaleString()}</b><br>최종 계급: <b>정직원</b><br>최대 콤보: <b>${game.maxCombo}</b>`,
    ...
    function showEndingIllustrationScreen() {
      ...
      endingMetaStats.innerHTML = `
        최종 점수: <b>${Math.floor(game.score).toLocaleString()}</b><br>
        최종 계급: <b>${RANKS[Math.min(game.rankIndex, RANKS.length - 1)]}</b><br>
        최대 콤보: <b>${game.maxCombo}</b><br>
        해금 여부: <b>완료</b>
      `;
    }

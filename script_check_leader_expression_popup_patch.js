    ...
    function getPortraitImagePath(person, expression = "normal") {
      ...
      // 수장님은 4가지 표정 전용 팝업을 사용합니다.
      // normal: 평상시, talk: 말할 때, surprised: 놀랄 때, angry: 화났을 때
      if (id === "leader" && portraitPaths.leader && portraitPaths.leader[expr]) {
        return portraitPaths.leader[expr];
      }

      if (portraitPaths[id] && portraitPaths[id][expr]) {
        return portraitPaths[id][expr];
      }
    ...
    // preload
    [...riceFrames, ...kimchiFrames, defaultImage, ...stageBackgrounds, ...agendaImages, ...Object.values(endingImages), ...portraitKeys.flatMap(k => portraitExpressions.map(e => portraitPaths[k][e])), ...Object.values(popupCardPaths), ...Object.values(portraitPaths.leader || {})].forEach(src => { const img = new Image(); img.src = src; });

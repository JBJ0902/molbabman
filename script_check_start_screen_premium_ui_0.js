    ...
    const endingImages = {
      ...
      riceOnly: "assets/endings/rice.png",
      silent: "assets/endings/silent.png",
      comboMaster: "assets/endings/combo.png",
      tightrope: "assets/endings/tight.png",
      comeback: "assets/endings/comeback.png",
    ...
    const ENDING_DETAILS = {
      ...
      comeback: {
        title: "막판 역전 정직원 엔딩",
        shortName: "역전",
        condition: "정직원 상태로 클리어 + 몰밥 미션 실패 2~4회",
        hint: "초반에 미션 실패가 누적되어도 5회가 되기 전에 후반 스테이지를 버텨내세요.",
        galleryDesc: "초반에는 위태로웠지만 후반 집중력으로 살아남았습니다. 몰밥 회의실의 막판 역전극입니다."
      },
      intern: {
        title: "인턴 생존 엔딩",
        shortName: "인턴",
    ...
    function determineEnding(clear) {
      ...
      if (regularRank && objectiveFailCount >= 2 && objectiveFailCount < 5) {
        unlockEnding("comeback");
        return {
          key: "comeback",
          title: "막판 역전 정직원 엔딩",
          desc: `초반에는 미션 실패가 누적되며 위태로웠지만, 후반 집중력으로 정직원 자리를 지켜냈습니다. 몰밥 회의실의 막판 역전극입니다.<br>${endingDescLine}${statLine}`,
          voice: "막판 역전 정직원 엔딩입니다. 끝까지 버텨낸 역전승입니다."
        };
      }
    ...
    window.addEventListener("keyup", e => {
      if (shouldBlockGameHotkeys(e)) return;
    ...
    function updateStartOnlineChip() {
      ...
      if (onlineState.user && onlineState.profile) {
        chip.textContent = `온라인 로그인: ${onlineState.profile.nickname} · ${onlineState.profile.tier || getOnlineTier(onlineState.profile.best_score)} · 접속자 ${onlineState.activeCount || 1}명`;
    ...
    function updateOnlineProfileUI() {
      ...
      if (!loggedIn) {
        ...
        setOnlineElementText("myTierBadge", "방출 후보");
        setOnlineElementText("myBestScore", "0");
        setOnlineElementText("myTotalPlays", "0");
        setOnlineElementText("myEndingCount", "0/10");
        setOnlineElementText("tierNextLabel", "다음 티어까지");
        setOnlineElementText("tierNextScore", "10,000점");
      ...
      const tier = profile.tier || getOnlineTier(best);
      const progress = getTierProgressInfo(best);

      setOnlineAuthStatus(`${profile.nickname || "몰밥러"} 계정으로 로그인됨 · ${tier}`, "ok");
      setOnlineElementText("myTierBadge", tier);
      setOnlineElementText("myBestScore", best.toLocaleString());
      setOnlineElementText("myTotalPlays", String(profile.total_plays || 0));
    ...
    async function loadOnlineRanking() {
      ...
      try {
        ...
        const htmlRows = data.map((row, index) => {
          ...
          const tier = row.tier || getOnlineTier(score);
          return `
            <div class="rankRow ${isMe ? "me" : ""}">
              <div class="rankNo">#${index + 1}</div>
              <div class="rankNick" title="${row.nickname || "몰밥러"}">${row.nickname || "몰밥러"}</div>

    ...
    function stageRequirementFail() {
      ...
      pulseWrap("danger");
      showResultBanner(game.objectiveFailCount >= 5 ? "로켓 방출!" : "딴짓 하다 경고!", "danger", 1250);
      spawnResultParticles("gameover");
      ...
      statusText.innerHTML = `목표 미달 · <b>${scoreNow}점</b> / 몰밥 <b>${game.eatCount || 0}회</b><br>몰밥 실패 <b>${failText}</b>`;
      ...
      if (game.objectiveFailCount >= 5) {
        ...
        statusText.innerHTML = `몰밥 실패 <b>5회</b> 누적 · 로켓 방출`;
        updateHUD();
      ...
      game.stageFailTimer = setTimeout(() => {
        ...
        statusText.textContent = `스테이지 ${game.stage} 재도전 · 실패 ${game.objectiveFailCount}/5`;
        updateHUD();
    ...
    function activateRice(active) {
      ...
      if (active) game.usedRiceKey = true;
    ...
    function activateKimchi(active) {
      if (!game.running || !optionsOverlay.classList.contains("hidden")) return;
      if (active) game.usedKimchiKey = true;

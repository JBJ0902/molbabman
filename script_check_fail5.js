    ...
    // 이렇게 해야 수장 시선 타이머와 긴 TTS 음성이 서로 충돌하지 않습니다.
    ...
    const bubblePeople = [
      { id: "leader", name: "수장", x: 49.6, y: 18.8, px: 40.6, py: 30.2, role: "leader", cls: "speaker-leader", pitch: 0.82, rate: 1.0 },
    ...
    const stageDialogue = {
      ...
      late: {
        ...
        center: [
          ...
          "오늘 최종 목표는 레전드 장면 생산과 수장 위신 회복입니다.",
    ...
    const ENDING_DETAILS = {
      perfect: {
        ...
        galleryDesc: "수장의 감시를 완벽하게 피하고 회의실 몰밥을 끝까지 성공한 전설의 엔딩입니다."
      },
      clear: {
        title: "생존 성공 엔딩",
        shortName: "생존",
        ...
        galleryDesc: "긴 회의와 수장의 시선을 버티고 끝까지 살아남은 기본 클리어 엔딩입니다."
      },
      kimchiKing: {
        title: "김치왕 엔딩",
        shortName: "김치왕",
    ...
    const dialogueState = {
      ...
      watchPending: false,
      watchPendingAt: 0,
      lastWatchDialogueAt: 0
    };

    // 수장 시선/판정은 기존 타이머대로 즉시 진행하되,
    // 음성 대화는 절대 중간에 끊지 않고 큐에 넣어 순서대로 재생합니다.
    const WATCH_DIALOGUE_PENDING_MAX_MS = 2400;
    const WATCH_DIALOGUE_COOLDOWN_MS = 8200;
    ...
    function canRunDialogueQueue() {
      return game.running && !["caught", "stageClear", "launching"].includes(game.phase);
    }

    function isWatchDialogueContextActive() {
      return game.running
        && game.phase === "watch"
        && (game.gazeMode === "player" || game.gazeMode === "playerWarn");
    }
    ...
    function requestWatchDialogueSequence() {
      // 이벤트 전용 음성 모드에서는 공태연 감시 구간의 긴 경고/변명 대화를 재생하지 않습니다.
      // WARNING 화면 + 효과음 + 실제 적발 음성만 사용해야 시선/자막/음성 싱크가 안정됩니다.
      if (HYBRID_AUDIO_EVENT_ONLY || !WATCH_DIALOGUE_USES_TEXT) return false;
      if (!isWatchDialogueContextActive()) return false;

      const now = performance.now();
      // 수장이 공태연을 실제로 보고 있는 짧은 WATCH 구간에서만 경고/변명 대화를 허용합니다.
      // 이미 다른 대사가 재생 중이면 큐에 넣되, WATCH가 끝나면 자동 폐기해서 엉뚱한 대상에게 말하지 않게 합니다.
      if (now - dialogueState.lastWatchDialogueAt < WATCH_DIALOGUE_COOLDOWN_MS) {
        return false;
      }
    ...
    function runPendingWatchDialogue() {
      ...
      // WATCH 구간이 끝났거나 수장이 더 이상 공태연을 보고 있지 않으면
    ...
    async function showConversationTurn() {
      ...
      try {
        ...
        if (Math.random() < 0.5) {
          ...
          await createBubble(bubblePeople[0], replyLine.text, MEETING_DIALOGUE_USES_VOICE, replyLine.source, { token, textOnlyVisibleMs: 2000 });
        }
      } finally {
    ...
    async function showWatchDialogueSequence() {
      ...
      try {
        ...
        await createBubble(bubblePeople[0], watchLine.text, WATCH_DIALOGUE_USES_VOICE, watchLine.source, { token, afterGapMs: 120, textOnlyVisibleMs: 1600 });
        if (!isDialogueTokenActive(token) || !isWatchDialogueContextActive()) return;

        const playerLine = pickPlayerReplyVoiceLine();
        await createBubble(playerSpeaker, playerLine.text, WATCH_DIALOGUE_USES_VOICE, playerLine.source, { token, afterGapMs: 160, textOnlyVisibleMs: 1700 });
      } finally {
        if (dialogueState.token === token) {
          dialogueState.busy = false;
          dialogueState.protectedSequence = false;
    ...
    function setSafePhase() {
      const cfg = getStageConfig(game.stage);
      clearWatchDialogueQueue();
      // WATCH 전용 대화가 아직 재생 중이면 다음 안전 구간으로 넘어가는 순간 폐기합니다.
      // 그래야 공태연 경고/변명이 다른 크루를 보는 화면에서 나오지 않습니다.
      if (dialogueState.protectedSequence) cancelDialogue(true, true);
      game.phase = "safe";
      ...
      statusText.textContent = game.stage <= 3
        ? "소음 경계도와 수장 시선 타이머는 별개입니다. 시선 타이머가 끝나면 WARNING이 뜹니다."
        : game.stage <= 6
          ? "중반부터는 소음 경계도도 빨리 오르고, 수장 시선 주기도 짧아집니다."
          : "후반은 고난도입니다. 소음은 낮아도 수장 시선 타이머를 반드시 보세요.";
      setStageMood();
      ...
      if (warningSub) warningSub.textContent = `곧 수장이 쳐다봅니다! 약 ${cfg.warningTime.toFixed(1)}초 안에 손을 떼세요`;
      hideEventBubble();
      statusText.textContent = "WARNING! 곧 수장이 공태연 캐릭터 쪽을 봅니다. 지금 손을 떼면 안전하고, 원하면 연타를 유지할 수도 있습니다.";
      setGaze("playerWarn");
      ...
      const cfg = getStageConfig(game.stage);
      // HYBRID: 수장이 공태연을 볼 때는 긴 경고/변명 음성을 재생하지 않고, 화면 경고와 실제 적발 음성만 사용합니다.
      if (HYBRID_AUDIO_EVENT_ONLY && dialogueState.busy && !dialogueState.protectedSequence) cancelDialogue(false, true);
      game.phase = "watch";
      game.phaseTimer = cfg.watchTime;
      warningOverlay.classList.remove("active");
      hideEventBubble();
      statusText.textContent = "위험! 수장이 공태연 캐릭터를 보고 있습니다. 지금 먹으면 바로 들킵니다.";
      setGaze("player");
    ...
    function caught() {
      if (game.phase === "caught" || game.phase === "launching") return;
      cancelDialogue(true);
      game.phase = "caught";
      ...
      screenFlash.classList.remove("hit"); void screenFlash.offsetWidth; screenFlash.classList.add("hit"); pulseWrap("danger"); showResultBanner(game.life <= 0 ? "ROCKET EJECT!" : "CAUGHT!", "danger", 1100); spawnResultParticles("gameover");
      warningOverlay.classList.remove("active");
      showEventBubble(game.life <= 0 ? "로켓 태워서 방출!" : `${RANKS[game.rankIndex]}으로 강등!`, true, true, bubblePeople[0], game.life <= 0 ? voiceClips.rocket : voiceClips.demote);
      statusText.textContent = game.life <= 0
        ? "최종 적발! 로켓에 실려 팀에서 방출됩니다."
        : `들켰습니다. 현재 계급: ${RANKS[game.rankIndex]}`;
      setDefaultImage();
      playSfx("caught", 0.9);
      resetCombo("콤보 끊김!");
    ...
    function stageRequirementFail() {
      if (!game.running || game.phase === "stageFail" || game.phase === "stageClear" || game.phase === "launching") return;
      ...
      game.objectiveFailCount = (game.objectiveFailCount || 0) + 1;
      if (game.rankIndex < RANKS.length - 1) game.rankIndex += 1;

      pulseWrap("danger");
      showResultBanner(game.objectiveFailCount >= 5 ? "로켓 방출!" : "목표 미달!", "danger", 1250);
      spawnResultParticles("gameover");
      playSfx(game.objectiveFailCount >= 5 ? "rocket" : "warning", 0.72);

      const scoreNow = Math.floor(game.score || 0).toLocaleString();
      const failText = `${game.objectiveFailCount} / 5`;
      statusText.innerHTML = `몰밥 실적 부족! 목표 <b>${req.minScore.toLocaleString()}점</b> / <b>${req.minEatCount}회</b>를 채우지 못했습니다.<br>현재 ${scoreNow}점 · 몰밥 ${game.eatCount || 0}회 · 목표 미달 <b>${failText}</b>`;

      updateHUD();

      if (game.objectiveFailCount >= 5) {
        game.finalTotalScore = Math.floor(getTotalScoreNow());
        statusText.innerHTML = `목표 미달 <b>5회</b> 누적! 몰밥 실적 부족으로 로켓 방출됩니다.`;
      ...
      game.stageFailTimer = setTimeout(() => {
        ...
        statusText.textContent = `스테이지 ${game.stage} 재도전. 목표 미달 ${game.objectiveFailCount}/5회. 제한 시간 안에 목표 점수와 몰밥 횟수를 채우세요.`;
        updateHUD();
    ...
    function stageClear() {
      ...
      statusText.textContent = `스테이지 ${game.stage} 회의 시작. 수장의 시선이 더 빨라집니다.`;
      setGaze(randomSafeGaze());
      ensureStageBgm();
      pulseWrap("clear");
    ...
    function determineEnding(clear) {
      if (!clear) {
        ...
        return {
          ...
          desc: `로켓에 실려 크루에서 방출되었습니다.<br>최종 점수: <b>${Math.floor(game.score).toLocaleString()}</b><br>최종 계급: <b>${RANKS[Math.min(game.rankIndex, RANKS.length - 1)]}</b><br>최대 콤보: <b>${game.maxCombo}</b>`,
    ...
    function showEndingIllustrationScreen() {
      ...
      const key = game.endingKey || game.endingInfo.key || "clear";
      const detail = ENDING_DETAILS[key] || { title: game.endingInfo.title, galleryDesc: "", condition: "조건 정보 없음", hint: "" };
      ...
      const scoreLabel = galleryOnly ? "최고 점수" : "최종 점수";
      const scoreValue = galleryOnly ? Math.floor(records.highScore) : Math.floor(game.score);
      const rankText = galleryOnly ? "기록 보관함" : RANKS[Math.min(game.rankIndex, RANKS.length - 1)];
      const comboValue = galleryOnly ? records.bestCombo : game.maxCombo;

      endingMetaStats.innerHTML = `
        ${scoreLabel}: <b>${scoreValue.toLocaleString()}</b><br>
        ${galleryOnly ? "보기 모드" : "최종 계급"}: <b>${rankText}</b><br>
        최대 콤보: <b>${comboValue}</b>
      `;
    ...
    function updateSuspicion(dt) {
      ...
      if (game.suspicion >= 100 && action !== "idle") {
        showEventBubble("소리가 너무 큽니다! 수장이 바로 확인합니다!", true, false, bubblePeople[0]);
        caught();
        return;
      }
    }
    ...
    function activateRice(active) {
      ...
      if (game.phase === "watch" && active) { caught(); return; }
      ...
      // 단, 수장이 실제로 쳐다보는 watch 상태에서 누르면 기존처럼 즉시 적발됩니다.
    ...
    function showStageStartCard(stage) {
      ...
      if (!card) return;
      const no = document.getElementById("stageStartNo");
      const tm = document.getElementById("stageStartTime");
      if (no) no.textContent = stage;
      if (tm) tm.textContent = formatClock(getStageConfig(stage).timeLimit);
      const info = card.querySelector(".stageStartInfo");
      const req = getStageRequirement(stage);
      if (info) {
        info.innerHTML = `제한 시간 <b>${formatClock(getStageConfig(stage).timeLimit)}</b> · 목표 <b>${req.minScore.toLocaleString()}점</b> · 몰밥 <b>${req.minEatCount}회</b>`;
      }
    ...
    function determineEnding(clear) {
      ...
      if (!clear) {
        unlockEnding("rocket");
        return { key: "rocket", title: game.stage >= 7 ? "장렬한 로켓 방출 엔딩" : "조기 방출 엔딩", desc: `로켓에 실려 크루에서 방출되었습니다.<br>최종 점수: <b>${finalScore.toLocaleString()}</b><br>최종 계급: <b>${RANKS[Math.min(game.rankIndex, RANKS.length - 1)]}</b><br>최대 콤보: <b>${game.maxCombo}</b>`, voice: "로켓 방출 엔딩입니다. 다음에는 더 조용히 드세요." };
      }
      if (game.life === 5 && game.rankIndex === 0) {
        unlockEnding("perfect");
    ...
    function showEndingIllustrationScreen() {
      ...
      const finalScore = Math.floor(game.finalTotalScore ?? getTotalScoreNow());
      endingIllustrationOverlay.classList.remove("hidden");
      endingArtTitle.textContent = game.endingInfo.title;
      endingMetaTitle.textContent = game.endingInfo.title;
      endingMetaDesc.innerHTML = game.endingInfo.desc;
      endingArtImage.src = endingImages[game.endingKey] || defaultImage;
      endingMetaStats.innerHTML = `최종 점수: <b>${finalScore.toLocaleString()}</b><br>최종 계급: <b>${RANKS[Math.min(game.rankIndex, RANKS.length - 1)]}</b><br>최대 콤보: <b>${game.maxCombo}</b><br>해금 여부: <b>완료</b>`;
    }
    function resetGame() {
      game.running = true;
      game.stage = 1;
    ...
    function update(dt) {
      if (!game.running) return;
      game.phaseTimer -= dt;
      if (!["stageClear", "stageFail", "launching"].includes(game.phase)) {
      ...
      updateLookTimerHUD();
      updateStageTimerHUD();
      if (game.stageTimeLeft <= 0 && !["stageClear", "stageFail", "launching"].includes(game.phase)) {
